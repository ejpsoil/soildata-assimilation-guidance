---
title: GRLC.io, setup a conviniance API on a SPARQL endpoint
...
API on top of a SPARQL endpoint, which can also be used to setup a conveniance API to browse through codelists. 

[GRLC](https://grlc.io) creates an [OpenAPI](https://spec.openapis.org/) facade on any SPARQL endpoint. Users send an OpenAPI
...
This query will select all `skos:concepts` in scheme `carbonOrganicProcedure`.
