...
The GML tools CLIs are described in further detail in the deegree [online documentation](https://download.deegree.org/documentation/3.4.32/html/#deegree-gml-tools). 

# Example workflow

The above mentioned GML tools mentioned above must be executed in the following order:
...
For beginners, the easiest way to configure deegree is via the web console at [http://localhost:8080/deegree-webservices](http://localhost:8080/deegree-webservices).
...
* The URL in the XML template needs to be adjusted according to the previously created PostgreSQL docker container as well as the database `'inspire_soil'`. The set URL should look like this `"jdbc:postgresql://your.ip:5432/inspire_soil"`. The previously chosen password and username also need to be adjusted. Click the button `save` to save your changes to the XML template.
> **Attention:** Use your IP address instead of localhost, so Docker can communicate with your local machine.
...
* Next, open the previosly created file `Soil.xml` and select all of its content (CTRL+A+C). 
...
> **Info:** You can also add the file `Soil.xml` directly into your workspace in `.deegree/yourworkspace/datasources/feature/` and modify it there.
...
After downloading and extracting the dataset mentioned at the beginning of this example workflow, navigate into the folder containing the file `Soil.gml`:

```
cd SO_BoKw2015/
```
Next, the file needs to be copied onto the previously created deegree docker container using the following docker command:
```
...
The capabilties should be then be accessible at the following address:
