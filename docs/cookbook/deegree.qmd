...
`docker cp deegree:/opt/Soil.xml /destination`.
...
* Next, open the previosly created file `Soil.xml` and select all of its content (CTRL+A, CTRL+C). 
...
> **Attention:** The deegree GmlLoader takes around 23-26 hours to run, since the dataset used in this example is quite large (854,2 MB).
